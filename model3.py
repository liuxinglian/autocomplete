...
def get_rnn_cell(typ='rnn', platform='cpu', **kwargs):
    ...
    if typ == 'rnn' and platform == 'cpu':
        ...
        return tf.contrib.cudnn_rnn.CudnnLSTMSaveable(**kwargs)
...
def build_nn(training, input_ph, n_steps, n_inputs, n_neurons, seq_length_ph, out_size=100, keep_prob=0.5):
    cell = get_rnn_cell(typ='rnn', platform='cpu', num_units = n_neurons)
...
def main():
    ...
    model, sentences = get_word_embedding(filename,0, 10000)
    #TODO: to change
    n_steps = 20
    reverse = True
    dataset = prepare_input_for_nn(model, sentences, n_steps, reverse)
    test_sentences = get_review_data(filename, 10000, 12000)
    ...
    nn_model = build_nn(training, input_ph, n_steps, n_inputs, n_neurons, seq_length_ph)
