...
def get_word_embedding(filename):
    ...
    saved_model = my_file = Path('fasttext.txt')

    if not saved_model.is_file():
        
        model = FastText(sentences, size=100, workers=8, sg=1, min_count=1)
        
        # save the trained model
        model.wv.save_word2vec_format('fasttext.txt')
        saved_model = my_file = Path('fasttext.txt')

    else:
        model = word2vec.KeyedVectors.load_word2vec_format('fasttext.txt', binary=False)
